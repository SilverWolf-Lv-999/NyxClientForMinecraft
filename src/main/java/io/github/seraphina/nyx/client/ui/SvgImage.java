...
public class SvgImage {
    ...
    @Getter
    private final Identifier svgLocation;
    ...
    private Identifier currentTextureLocation;
    private final Map<Long, Identifier> animationFrameLocations = new HashMap<>();
    ...
    public SvgImage(Identifier svgLocation) {
        this(svgLocation, 256, 256);
    }

    public SvgImage(Identifier svgLocation, int textureWidth, int textureHeight) {
    ...
    public void update() {
        ...
        try {
            ...
            failed = true;
            NyxClient.LOGGER.error("Failed to update SVG image {}", svgLocation, exception);
        }
    }
    ...
    public void restartAnimation() {
        ...
        if (loaded && !failed) {
            try {
                ...
                failed = true;
                NyxClient.LOGGER.error("Failed to restart SVG image {}", svgLocation, exception);
            }
        }
    }
    ...
    private static void drawTexturedQuad(GuiGraphics graphics, Identifier textureLocation,
                                         float x, float y, float width, float height, float alpha, Color tint) {
        Color safeTint = tint != null ? tint : Color.WHITE;
        int baseColor = safeTint.getRGB();
        int renderColor = (baseColor & 0x00FFFFFF) | (Math.round(((baseColor >>> 24) & 0xFF) * clamp01(alpha)) << 24);
        Render2DUtility.withGuiGraphics(graphics, () -> Render2DUtility.drawTexture(
                Minecraft.getInstance().getTextureManager().getTexture(textureLocation).getTextureView(),
                x,
                y,
                width,
                height,
    ...
    private void ensureLoaded() {
        ...
        try {
            ...
            failed = true;
            NyxClient.LOGGER.error("Failed to load SVG image {}", svgLocation, exception);
        }
    }
    ...
    private void uploadRasterizedFrame(float timeSeconds) {
        ...
        Runnable upload = () -> {
            if (texture == null) {
                DynamicTexture dynamicTexture = new DynamicTexture(() -> "nyx-svg-" + textureLocation, nativeImage);
    ...
    private Identifier textureLocationForFrame(long frameKey, float timeSeconds) {
        if (repeatAnimation) {
    ...
    private Identifier animationFrameTextureLocation(long frameKey) {
