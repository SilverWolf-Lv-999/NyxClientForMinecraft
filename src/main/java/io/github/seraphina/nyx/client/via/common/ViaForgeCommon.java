...
package io.github.seraphina.nyx.client.via.common;
...
import io.github.seraphina.nyx.client.via.common.platform.ViaForgeConfig;
import io.github.seraphina.nyx.client.via.common.platform.ViaForgePlatform;
import io.github.seraphina.nyx.client.via.common.protocoltranslator.platform.ViaForgePlatformLoader;
import io.github.seraphina.nyx.client.via.common.protocoltranslator.platform.ViaForgeViaVersionPlatform;
...
public class ViaForgeCommon {
    ...
    public static final AttributeKey<UserConnection> VF_VIA_USER = AttributeKey.valueOf("nyx_via_user");
    public static final AttributeKey<ExtendedNetworkManager> VF_NETWORK_MANAGER = AttributeKey.valueOf("nyx_via_network_manager");
    ...
    public static void init(final ViaForgePlatform platform) {
        ...
        final File mainFolder = new File(platform.getLeadingDirectory(), "protocol");
        ...
        manager.config = new ViaForgeConfig(new File(mainFolder, "config.yml"), Via.getPlatform().getLogger());
    ...
    public void inject(final Channel channel, final ExtendedNetworkManager networkManager) {
        final ProtocolVersion targetVersion = networkManager.viaForge$getTrackedVersion();
        if (targetVersion == null || targetVersion.getVersion() == getNativeVersion().getVersion()) {
            return;
        }
    ...
    public void sendConnectionDetails(final Channel channel) {
        final UserConnection user = channel.attr(VF_VIA_USER).get();
        if (user != null) {
